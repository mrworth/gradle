...
abstract class AbstractNativeParallelIntegrationTest extends AbstractInstalledToolChainIntegrationSpec {
    ...
    def withTaskThatRunsParallelUsingToolchainWith(String taskName) {
    ...
    def withTaskThatRunsParallelUsingInjectionWith(String taskName) {
        setupParallelTaskAndExpectations(taskName)

        buildFile << """
            import org.gradle.api.internal.AbstractTask

            ${callbackServiceRegistry}

            tasks.matching { it.name == '${taskName}' }.all { task ->
                def serviceRegistry = AbstractTask.class.getDeclaredField("services")
                serviceRegistry.setAccessible(true)
                serviceRegistry.set(task, new CallbackServiceRegistry(getServices(), beforeOperations, afterOperations))
            }
        """
    }
    ...
    String getCallbackServiceRegistry() {
        return """
            import org.gradle.internal.service.ServiceRegistry

            ${callbackWorkerLeaseService}

            class CallbackServiceRegistry implements ServiceRegistry {
                @Delegate
                ...
                CallbackServiceRegistry(ServiceRegistry delegate, Closure beforeCallback, Closure afterCallback) {
                    this.delegate = delegate
                    this.beforeCallback = beforeCallback
                    this.afterCallback = afterCallback
                }
                ...
                public <T> T get(Class<T> serviceClass) {
                    if (serviceClass == WorkerLeaseService.class) {
                        return new CallbackWorkerLeaseService(delegate.get(WorkerLeaseService.class), beforeCallback, afterCallback)
                    } else {
                        return delegate(serviceClass)
                    }
                }
            }
        """
